<template>
  <view class="index-wrapper">
    <view class="index-top-wrapper">
      ...
      <view class="index-top-left" v-if="isShowOperating">
        <van-checkbox
          checked-color="#696969"
          :value="isAllCheck"
          @change="allCheckChange()"
          >全选</van-checkbox
        >
      </view>
      <view class="index-top-center" v-if="isShowOperating" @click="deleteItem">
        <van-icon name="delete-o" /><text>删除</text>
      </view>
      <view class="index-top-right" @click="isShowOperating = !isShowOperating">
        <van-icon name="setting-o" /><text>{{
          !isShowOperating ? "操作" : "完成"
        }}</text>
      </view>
    </view>
    <view class="index-content">
      ...
      <view v-if="bookingDetailsList.length > 0">
        <view
          ...
          :key="item.objectId"
        >
          <view class="detail-l-icon" v-show="isShowOperating">
            <van-checkbox
              checked-color="#696969"
              :value="item.isCheck"
          ...
          <view class="detail-l">
            ...
            <view class="detail-l-titBg">{{
              item.saveMoney_details.expendDetail
            }}</view>
          </view>
          <view class="detail-c">{{
            item.saveMoney_details.beizhuinputVal
          }}</view>
          <view
            :class="item.zhichuMoney > 0 ? 'green detail-r ' : 'detail-r'"
            >{{ item.zhichuMoney }}</view
          >
        </view>
      </view>

      <!-- </van-skeleton> -->
      <van-empty description="暂无数据" v-if="bookingDetailsList.length == 0" />
    </view>
  ...
  methods: {
    handleShowTips() {
      uni.showModal({
        title: "温馨提示",
        content:
          "由于外部环境及运营成本的上浮，记账功能将于2022年5月5日下线，届时将停止记账功能，对于之前的数据会予以保留，待优化产品后择期重新上线，感谢您的信任与理解！",
        showCancel: false,
    ...
    getWXLoginCode() {
      ...
      uni.login({
        ...
        success: function (loginRes) {
          ...
          // 获取用户信息
          that.Bmob.User.loginWithWeapp(loginRes.code).then(function (user) {
            //o9Dy55SOyX7I85d_KFCT1DMuDxDk
            // 可以拿到openid
            console.log("获取用户的openid", user.authData.weapp.openid);
